...
function SuperadminAnalyticsSkeleton() {
  return (
    <div aria-label="Memuat perbandingan sekolah" className="grid gap-4 lg:grid-cols-2">
      {[1, 2].map((item) => (
  ...
  return (
    <div className="flex min-w-0 flex-col gap-6 overflow-x-hidden p-4 sm:p-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-semibold text-slate-900 dark:text-white">
              Analitik antar sekolah
            </h1>
            <p className="mt-1 max-w-2xl text-sm text-slate-500">
              Snapshot perbandingan hasil akademik seluruh sekolah dalam riset.
            </p>
          </div>
        </div>
      </header>
      ...
      <Alert status="accent">
        ...
        <Alert.Content>
          <Alert.Description>
            Perbandingan menggunakan skor mentah, digabung lintas seluruh guru
            dalam satu sekolah. Attempt dengan esai pending belum masuk
            statistik skor final.
          </Alert.Description>
        </Alert.Content>
      </Alert>
      ...
      {comparison.isLoading ? <SuperadminAnalyticsSkeleton /> : null}
      ...
      comparisons.length === 0 ? (
        <Card className="items-center py-12 text-center">
          ...
          <Card.Title>Belum ada sekolah</Card.Title>
          <Card.Description>
            Tambahkan sekolah terlebih dahulu agar perbandingan dapat
            ditampilkan.
          </Card.Description>
        </Card>
      ) : null}
      ...
      comparisons.length > 0 ? (
        <>
          {hasFinalScores ? (
            <section
              aria-label="Visualisasi perbandingan sekolah"
              className="grid min-w-0 grid-cols-1 gap-4 lg:grid-cols-2"
            >
              <SchoolAverageScoreChart comparisons={comparisons} />
              <SchoolScoreDistributionChart comparisons={comparisons} />
            </section>
          ) : (
            <Alert status="warning">
              ...
              <Alert.Content>
                <Alert.Description>
                  ...
                  penilaian esai tetap terlihat pada detail sekolah.
                </Alert.Description>
              </Alert.Content>
            </Alert>
          )}
