...
export function EditSchoolForm({ schoolId, currentName, onUpdated }: EditSchoolFormProps) {
  const [values, setValues] = useState<CreateSchoolFormValues>({ name: currentName });
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await updateSchool.mutateAsync({ id: schoolId, name: parsed.data.name });
      toast.success(`Sekolah "${parsed.data.name}" berhasil diperbarui.`);
      onUpdated();
    } catch (error) {
      ...
      setFormAlert({ message: "Gagal memperbarui sekolah. Silakan coba lagi." });
    }
  }
  ...
  return (
    <Form onSubmit={handleSubmit} className="flex w-full flex-col gap-4">
      ...
      <TextField
        ...
        isDisabled={updateSchool.isPending}
      >
      ...
      <Button
        type="submit"
        isPending={updateSchool.isPending}
        isDisabled={updateSchool.isPending}
        fullWidth
